...
namespace IoTLogic.Core.Reflection
{
    public static class RuntimeCodebase
    {
        ...
        */
        ...
        {
            Ensure.Ensure.That(nameof(attributeType)).IsNotNull(attributeType);
            Ensure.Ensure.That(nameof(assemblies)).IsNotNull(assemblies);
        ...
        private static Dictionary<string, string> FetchRenamedMembers(Type type)
        {
            ...
            foreach (var member in members)
            {
                ...
                {
                    Console.WriteLine($"Failed to fetch RenamedFrom attributes for member '{member}':\n{ex}");
                    continue;
                }
                ...
                foreach (var renamedFromAttribute in renamedFromAttributes)
                {
                    ...
                    {
                        Console.WriteLine($"Multiple members on '{type}' indicate having been renamed from '{previousMemberName}'.\nIgnoring renamed attributes for '{member}'.");
